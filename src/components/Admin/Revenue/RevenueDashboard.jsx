...
import { TrendingUp, TrendingDown, Users, DollarSign, Calendar, Filter, ChevronLeft, ChevronRight } from 'lucide-react';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';
import { getRevenueDashboard, getRevenueDashboardWithDateRange } from '../../../apiHandler/adminAPIHandler';
...
const RevenueDashboard = () => {
  ...
  const statistics = useMemo(() => {
    ...
    const pieChartData = revenueByCategory.map((item, index) => ({
      name: item.category,
      value: item.percent,
      color: item.category === 'Subscription' ? '#8B5CF6' : 
             item.category === 'Consultation' ? '#06B6D4' : 
             item.category === 'Service Fee' ? '#EC4899' : '#F59E0B'
    }));
  ...
  const totalPages = Math.ceil((dashboardData?.recentTransactions?.length || 0) / transactionsPerPage);
  const indexOfLastTransaction = currentPage * transactionsPerPage;
  const indexOfFirstTransaction = indexOfLastTransaction - transactionsPerPage;
  const currentTransactions = dashboardData?.recentTransactions?.slice(indexOfFirstTransaction, indexOfLastTransaction) || [];
  ...
  const CategoryBadge = ({ category }) => {
    const getColor = (cat) => {
      switch (cat) {
        case 'Subscription': return 'badge-purple';
        case 'Consultation': return 'badge-blue';
        case 'Service Fee': return 'badge-pink';
  ...
  const handleResetFilter = () => {
    ...
    setCustomDateRange({ startDate: defaultStartDate, endDate: defaultEndDate });
  ...
  return (
    <div className="dashboard">
              ...
              <PieChart>
                ...
                >
                  {statistics.pieChartData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={entry.name === 'Subscription' ? 'url(#pieGradient1)' : 
                            entry.name === 'Consultation' ? 'url(#pieGradient2)' : 
                            entry.name === 'Service Fee' ? 'url(#pieGradient3)' : 'url(#pieGradient4)'} 
                    />
                  ))}
                </Pie>
              </PieChart>
              ...
              {statistics.pieChartData.map((item, index) => (
                <div key={index} className="legend-item">
                  <div 
                    ...
                    style={{ 
                      background: item.name === 'Subscription' ? 'linear-gradient(135deg, #8B5CF6, #7C3AED)' : 
                                 item.name === 'Consultation' ? 'linear-gradient(135deg, #06B6D4, #0891B2)' : 
                                 item.name === 'Service Fee' ? 'linear-gradient(135deg, #EC4899, #DB2777)' : 
      ...
      <div className="transactions-section">
        ...
        <div className="transactions-table">
          ...
          {currentTransactions.map((transaction, index) => (
            <div key={index} className="table-row">
              ...
              <div className="transaction-amount">{formatCurrency(Math.abs(transaction.amount))}</div>
        ...
        {/* Pagination */}
        {dashboardData?.recentTransactions && dashboardData.recentTransactions.length > transactionsPerPage && (
